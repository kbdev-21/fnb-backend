...
//        var category = categoryService.getCategoryById(dto.getCategoryId());
    ...
    public ProductDto deleteProduct(UUID productId) {
        Product product = productRepository.findById(productId).orElseThrow(() -> new DomainException(DomainExceptionCode.PRODUCT_NOT_FOUND));
    ...
    private ProductDto mapToDtoFromEntity(Product product) {
        ...
        var categoryDto = categoryService.getCategoryById(product.getCategory().getId());
    ...
    private List<ProductDto> mapToDtosFromEntities(List<Product> products) {
        ...
        return products.stream()
            .map(product -> {
                ProductDto dto = modelMapper.map(product, ProductDto.class);
                dto.setCategory(categoryMap.get(product.getCategory().getId()));
                return dto;
            })
            .toList();
    }
